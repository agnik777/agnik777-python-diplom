...
class ProductUtils:
    ...
    def exclude_expired_products(queryset):
        """Исключает просроченные товары из queryset"""
    ...
    def calculate_order_total(order_items) -> int:
        ...
        return sum(
            ProductUtils.calculate_item_total(item)
...
class OrderUtils:
    ...
    def add_item_to_basket(basket, product) -> Dict[str, Any]:
        ...
        order_item = basket.order_items.filter(product=product).first()
        ...
        if order_item:
            ...
            if product.quantity >= 1:
                order_item = OrderItem.objects.create(
                    order=basket, product=product, quantity=1
                )
                return {'success': True,
    ...
    @staticmethod
    def _get_shops_data(order) -> Dict[str, Dict]:
        """Группирует товары заказа по магазинам"""
        shops_data = {}
        for item in order.order_items.all():
            shop = item.product.shop
            # if shop.owner.email not in shop_data:
            if shop.id not in shops_data:
                # shop_data[shop.owner.email] = {
                shops_data[shop.id] = {
                    'shop': shop,
                    'items': [],
                    'total': 0
            ...
            shops_data[shop.id]['total'] += (ProductUtils
...
class EmailUtils:
    ...
    def _format_order_items(order_items) -> List[str]:
        ...
        return [
            f"- {item.product.full_name}: {item.quantity} шт. × {
    ...
    def send_order_created_email(user, order, contact, phone) -> None:
        ...
        items_text = "\n".join(EmailUtils._format_order_items(
...
"""
    ...
    @staticmethod
    def send_shop_order_email(shop_owner, shop, order,
                              items, shop_total) -> None:
        """Email владельцу магазина о новом заказе"""
...
"""
    ...
    @staticmethod
    def send_order_status_changed_email(user, order,
                                        previous_status: str) -> None:
